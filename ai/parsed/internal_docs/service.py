...
def _office_binary() -> str:
    ...
    for candidate in candidates:
        ...
        if resolved:
            return resolved
    raise FileNotFoundError("LibreOffice/soffice was not found. Install LibreOffice or set LIBREOFFICE_BIN.")
...
def _convert_docx_to_pdf(docx_path: Path, pdf_dir: Path) -> Path:
    ...
    result = subprocess.run(command, capture_output=True, text=True, timeout=int(os.getenv("DOCX_CONVERT_TIMEOUT_SEC", "180")))
    ...
    if not pdf_path.exists():
        matches = sorted(pdf_dir.glob(f"{docx_path.stem}*.pdf"))
        if not matches:
            raise RuntimeError(f"DOCX conversion succeeded but PDF was not found for {docx_path.name}")
        pdf_path = matches[0]
    return pdf_path
